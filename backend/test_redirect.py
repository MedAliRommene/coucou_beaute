...
def test_redirect():
    ...
    try:
        ...
        # 3. Créer un rendez-vous de test avec timezone
        ...
        # 4. Tester la vue HTTP avec suivi des redirections
        print("4. Test de la vue HTTP avec suivi des redirections...")
        ...
        if login_success:
            ...
            # Accéder à la page des rendez-vous SANS suivre les redirections
            response = client.get('/client/appointments/', follow=False)
            
            print(f"   - Status code: {response.status_code}")
            print(f"   - Location header: {response.get('Location', 'N/A')}")
            ...
            if response.status_code == 302:
                ...
                print(f"   - Status code après redirection: {response.status_code}")
                print(f"   - URL finale: {response.request['PATH_INFO']}")
                
                if response.status_code == 200:
                    print("   ✓ Page chargée après redirection")
                    ...
                    print(f"\n5. Extrait du HTML (premiers 1000 caractères):")
                    print(content[:1000])
